...
def main(config):
    ...
    trainset = VideoDataset_NR(videos_dir, datainfo, transformations_train, 448, is_train = True)
    testset = VideoDataset_NR(videos_dir, datainfo, transformations_test, 448, is_train = False)
    ...
    for epoch in range(config.epochs):
        ...
        with torch.no_grad():
            ...
            for i, (video, mos, _) in enumerate(test_loader):
    ...
    np.save(os.path.join(config.results_path, config.model_name + '_' + config.database + '_NR_v'+ str(config.exp_version)), best_test)
